...
func (x *Acknowledgement) GetAckType() Acknowledgement_AckType {
	if x != nil {
		return x.AckType
	}
...
func (x *Result) GetHighestBidFinal() int32 {
	if x != nil {
		return x.HighestBidFinal
	}
	return 0
}

// An outcome: if the auction is still alive, returns highest bid so far,
// otherwise returns the Result, including bidder and the final highest bid
type Outcome struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to OutcomeType:
	//
	//	*Outcome_HighestBidSoFar
	//	*Outcome_Result
...
func (x *Outcome) GetHighestBidSoFar() int32 {
	if x != nil {
		if x, ok := x.OutcomeType.(*Outcome_HighestBidSoFar); ok {
			return x.HighestBidSoFar
		}
	}
	return 0
}
...
type Outcome_HighestBidSoFar struct {
	HighestBidSoFar int32 `protobuf:"varint,1,opt,name=highest_bid_so_far,json=highestBidSoFar,proto3,oneof"`
}
...
func (*Outcome_HighestBidSoFar) isOutcome_OutcomeType() {}
...
const file_proto_proto_rawDesc = "" +
	...
	"\x0fAcknowledgement\x122\n" +
	"\aackType\x18\x01 \x01(\x0e2\x18.Acknowledgement.AckTypeR\aackType\"/\n" +
	"\aAckType\x12\r\n" +
